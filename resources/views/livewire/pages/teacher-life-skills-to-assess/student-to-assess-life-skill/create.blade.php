<div>
    <x-mary-header title="{{ __('Evaluación') }}"
        subtitle="{{ $studenttoassess->student->first_name }} {{ $studenttoassess->student->middle_name ?? '' }} {{ $studenttoassess->student->last_name1 }} {{ $studenttoassess->student->last_name2 }}">
        <x-slot:actions>
            <x-mary-button icon="o-arrow-left" class="btn-neutral"
                link="{{ route('students.to.assess.life.skills.show', $studenttoassess->teacher_life_skills_to_assess_id) }}">
                {{ __('Regrear') }}
            </x-mary-button>
        </x-slot:actions>
    </x-mary-header>
    ...
    <x-mary-card shadow>
        <x-mary-form wire:submit="store" class="space-y-6">
                        ...
                        <tr class="border-t border-gray-300 dark:border-gray-600">
                            ...
                            @for ($i = 1; $i <= 4; $i++)
                                <td class="p-2 border-r align-middle">
                                    <x-mary-radio wire:model="lifeskillsSelected.{{ $skill->name }}"
                                        :options="[['id' => $i, 'name' => '']]" />
                                </td>
                            @endfor
                        </tr>
            ...
            <x-slot:actions class="flex justify-end gap-4">
                <x-mary-button label="{{ __('Finalizar evaluación') }}" class="btn-primary" type="submit"
                    spinner="store" />
                <x-mary-button
                    link="{{ route('students.to.assess.life.skills.show', $studenttoassess->teacher_life_skills_to_assess_id) }}"
